...
We could use this for subsequent visualizations. This code chunk uses a strange feature of factor variables to order `State`. We first put `name` in order of appearance in our dataset, and the group by `name` and `year`. After that grouping, we can use `fct_reorder` to order `State` by the `percent` variable -- but because the resulting factor variable must have the same levels and ordering, the first combination of `name` and `year` gets used throughout. This is ... weird, and I don't exactly recommend it, but ... it works?
